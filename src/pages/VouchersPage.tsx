...
import { Printer, CreditCard, Plus, Trash2, Download } from "lucide-react";
...
export default function VouchersPage() {
  ...
  const generateVouchers = () => {
    ...
    for (let i = 0; i < count; i++) {
      newCards.push({
        ...
        profile: selectedProfile || profiles[0]?.name || "default",
      });
    }
    setCards(newCards);
  ...
  const pushToRouter = async () => {
    ...
    for (const card of cards) {
      try {
        await actionFn.mutateAsync({
          action: "add",
          data: { name: card.username, password: card.password, profile: card.profile, group: card.profile },
        });
        success++;
      } catch {
        failed++;
    ...
    printWindow.document.write(`
      ...
      <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Cairo', sans-serif; }
        ...
        .card-title { font-weight: 700; font-size: 11px; color: #1a1814; margin-bottom: 2px; }
        .card-sub { font-size: 9px; color: #8a7f72; margin-bottom: 8px; }
        .field { margin-bottom: 4px; }
        .label { font-size: 8px; color: #8a7f72; }
        .value { font-size: 12px; font-weight: 600; color: #1a1814; font-family: 'JetBrains Mono', monospace; letter-spacing: 1px; }
        .profile-badge {
          display: inline-block; margin-top: 6px; padding: 2px 8px;
          background: #e8e2d9; border-radius: 4px; font-size: 8px; color: #5a5247;
        }
        @media print {
          body { padding: 5mm; }
          .grid { gap: 3mm; }
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-bold text-foreground">توليد الكروت</h2>
