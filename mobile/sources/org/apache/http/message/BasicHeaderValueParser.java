...
public class BasicHeaderValueParser implements HeaderValueParser {
    ...
    public NameValuePair parseNameValuePair(CharArrayBuffer buffer, ParserCursor cursor, char[] delimiters) {
        ...
        if (charArrayBuffer == null) {
            ...
            while (true) {
                if (pos < indexTo) {
                    char ch = charArrayBuffer.charAt(pos);
                    if (ch == '=') {
                        break;
            ...
            if (pos == indexTo) {
                ...
                name = charArrayBuffer.substringTrimmed(indexFrom, pos);
                pos++;
            }
            if (terminated) {
            ...
            while (true) {
                ...
                char ch2 = charArrayBuffer.charAt(pos);
                boolean z = false;
                if (ch2 == '\"' && !escaped) {
                    qouted = !qouted;
                }
            ...
            int i2 = pos;
            while (i1 < i2 && HTTP.isWhitespace(charArrayBuffer.charAt(i1))) {
                i1++;
            }
            while (i2 > i1 && HTTP.isWhitespace(charArrayBuffer.charAt(i2 - 1))) {
                i2--;
            }
            if (i2 - i1 >= 2 && charArrayBuffer.charAt(i1) == '\"' && charArrayBuffer.charAt(i2 - 1) == '\"') {
                i1++;
                i2--;
            }
            String value = charArrayBuffer.substring(i1, i2);
            if (terminated) {
                pos++;
            }
            parserCursor.updatePos(pos);
