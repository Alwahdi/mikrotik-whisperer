...
public class IdleConnectionHandler {
    ...
    public void closeIdleConnections(long idleTime) {
        ...
        if (this.log.isDebugEnabled()) {
            Log log2 = this.log;
            log2.debug("Checking for connections, idle timeout: " + idleTimeout);
        }
        for (Map.Entry<HttpConnection, TimeValues> entry : this.connectionToTimes.entrySet()) {
            HttpConnection conn = entry.getKey();
            long connectionTime = entry.getValue().timeAdded;
            if (connectionTime <= idleTimeout) {
                if (this.log.isDebugEnabled()) {
                    Log log3 = this.log;
                    log3.debug("Closing idle connection, connection time: " + connectionTime);
                }
                try {
                    conn.close();
