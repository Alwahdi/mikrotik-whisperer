...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2, NestedScrollingChild3 {
    ...
    public static void setDebugAssertionsEnabled(boolean debugAssertionsEnabled) {
        sDebugAssertionsEnabled = debugAssertionsEnabled;
    }
    ...
    public RecyclerView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        boolean z = a2.getBoolean(R.styleable.RecyclerView_fastScrollEnabled, false);
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            TypedArray a3 = context2.obtainStyledAttributes(attributeSet, iArr2, i, 0);
            int i2 = descendantFocusability;
            ViewCompat.saveAttributeDataForStyleable(this, context, iArr2, attrs, a3, defStyleAttr, 0);
            nestedScrollingEnabled = a3.getBoolean(0, true);
            a3.recycle();
            TypedArray typedArray = a3;
        } else {
                ...
                if (vh != null) {
                    if (vh.isTmpDetached() || vh.shouldIgnore()) {
                        if (RecyclerView.sVerboseLoggingEnabled) {
                            Log.d(RecyclerView.TAG, "reAttach " + vh);
                        }
                    ...
                    throw new IllegalArgumentException("No ViewHolder found for child: " + child + ", index: " + index + RecyclerView.this.exceptionLabel());
                }
            ...
            public void detachViewFromParent(int offset) {
                ...
                if (view != null) {
                    ...
                    if (vh != null) {
                        if (!vh.isTmpDetached() || vh.shouldIgnore()) {
                            if (RecyclerView.sVerboseLoggingEnabled) {
                                Log.d(RecyclerView.TAG, "tmpDetach " + vh);
                            }
                            vh.addFlags(256);
                        } else {
                    ...
                    throw new IllegalArgumentException("No view at offset " + offset + RecyclerView.this.exceptionLabel());
                }
    ...
    public void setScrollState(int state) {
        if (state != this.mScrollState) {
            if (sVerboseLoggingEnabled) {
                Log.d(TAG, "setting scroll state to " + state + " from " + this.mScrollState, new Exception());
            }
    ...
    private void nestedScrollByInternal(int x, int y, @Nullable MotionEvent motionEvent, int type) {
        ...
        if (layoutManager == null) {
            ...
            float verticalDisplacement = motionEvent2 == null ? ((float) getHeight()) / 2.0f : motionEvent.getY();
            float horizontalDisplacement = motionEvent2 == null ? ((float) getWidth()) / 2.0f : motionEvent.getX();
            int x3 = i - releaseHorizontalGlow(i, verticalDisplacement);
            int y3 = i2 - releaseVerticalGlow(i2, horizontalDisplacement);
            startNestedScroll(nestedScrollAxis, i3);
            if (dispatchNestedPreScroll(canScrollHorizontal ? x3 : 0, canScrollVertical ? y3 : 0, this.mReusableIntPair, this.mScrollOffset, type)) {
            ...
            int i5 = canScrollHorizontal ? x2 : 0;
            if (canScrollVertical) {
                i4 = y2;
            }
            scrollByInternal(i5, i4, motionEvent2, i3);
            GapWorker gapWorker = this.mGapWorker;
            if (!(gapWorker == null || (x2 == 0 && y2 == 0))) {
                gapWorker.postFromTraversal(this, x2, y2);
            }
            stopNestedScroll(i3);
        }
    }
    ...
    private int releaseHorizontalGlow(int deltaX, float y) {
        float consumed = 0.0f;
        float displacement = y / ((float) getHeight());
        float pullDistance = ((float) deltaX) / ((float) getWidth());
        ...
        if (edgeEffect == null || EdgeEffectCompat.getDistance(edgeEffect) == 0.0f) {
            ...
            if (!(edgeEffect2 == null || EdgeEffectCompat.getDistance(edgeEffect2) == 0.0f)) {
                if (canScrollHorizontally(1)) {
                    ...
                    consumed = EdgeEffectCompat.onPullDistance(this.mRightGlow, pullDistance, displacement);
            ...
            if (canScrollHorizontally(-1)) {
                ...
                consumed = -EdgeEffectCompat.onPullDistance(this.mLeftGlow, -pullDistance, 1.0f - displacement);
        ...
        return Math.round(((float) getWidth()) * consumed);
    }
    ...
    private int releaseVerticalGlow(int deltaY, float x) {
        ...
        if (edgeEffect == null || EdgeEffectCompat.getDistance(edgeEffect) == 0.0f) {
            ...
            if (!(edgeEffect2 == null || EdgeEffectCompat.getDistance(edgeEffect2) == 0.0f)) {
                if (canScrollVertically(1)) {
                    ...
                    consumed = EdgeEffectCompat.onPullDistance(this.mBottomGlow, pullDistance, 1.0f - displacement);
            ...
            if (canScrollVertically(-1)) {
                ...
                consumed = -EdgeEffectCompat.onPullDistance(this.mTopGlow, -pullDistance, displacement);
        ...
        return Math.round(((float) getHeight()) * consumed);
    }
    ...
    public boolean fling(int velocityX, int velocityY) {
        ...
        if (layoutManager == null) {
            ...
            if (!(flingX == 0 && flingY == 0)) {
                ...
                flingX = Math.max(-i, Math.min(flingX, i));
                int i2 = this.mMaxFlingVelocity;
                flingY = Math.max(-i2, Math.min(flingY, i2));
            ...
            if (velocityX != 0 || velocityY != 0) {
                if (!dispatchNestedPreFling((float) velocityX, (float) velocityY)) {
                    ...
                    if (canScroll) {
                        ...
                        int velocityX2 = Math.max(-i3, Math.min(velocityX, i3));
                        int i4 = this.mMaxFlingVelocity;
                        this.mViewFlinger.fling(velocityX2, Math.max(-i4, Math.min(velocityY, i4)));
                        return true;
                    }
                }
                return false;
            } else if (flingX == 0 && flingY == 0) {
                return false;
            } else {
                return true;
            }
    ...
    private boolean shouldAbsorb(@NonNull EdgeEffect edgeEffect, int velocity, int size) {
        ...
        if (getSplineFlingDistance(-velocity) < EdgeEffectCompat.getDistance(edgeEffect) * ((float) size)) {
            return true;
        }
        return false;
    }
    ...
    /* access modifiers changed from: package-private */
    public int consumeFlingInHorizontalStretch(int unconsumedX) {
    ...
    private int consumeFlingInStretch(int unconsumed, EdgeEffect startGlow, EdgeEffect endGlow, int size) {
        if (unconsumed > 0 && startGlow != null && EdgeEffectCompat.getDistance(startGlow) != 0.0f) {
            int consumed = Math.round((((float) (-size)) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(startGlow, (((float) (-unconsumed)) * FLING_DESTRETCH_FACTOR) / ((float) size), 0.5f));
            if (consumed != unconsumed) {
                startGlow.finish();
            }
            return unconsumed - consumed;
        } else if (unconsumed >= 0 || endGlow == null || EdgeEffectCompat.getDistance(endGlow) == 0.0f) {
            return unconsumed;
        } else {
            int consumed2 = Math.round((((float) size) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(endGlow, (((float) unconsumed) * FLING_DESTRETCH_FACTOR) / ((float) size), 0.5f));
    ...
    private void pullGlows(float x, float overscrollX, float y, float overscrollY) {
        ...
        if (overscrollX < 0.0f) {
            ensureLeftGlow();
            EdgeEffectCompat.onPullDistance(this.mLeftGlow, (-overscrollX) / ((float) getWidth()), 1.0f - (y / ((float) getHeight())));
            invalidate = true;
        } else if (overscrollX > 0.0f) {
            ensureRightGlow();
            EdgeEffectCompat.onPullDistance(this.mRightGlow, overscrollX / ((float) getWidth()), y / ((float) getHeight()));
            invalidate = true;
        }
        if (overscrollY < 0.0f) {
            ensureTopGlow();
            EdgeEffectCompat.onPullDistance(this.mTopGlow, (-overscrollY) / ((float) getHeight()), x / ((float) getWidth()));
            invalidate = true;
        } else if (overscrollY > 0.0f) {
            ensureBottomGlow();
            EdgeEffectCompat.onPullDistance(this.mBottomGlow, overscrollY / ((float) getHeight()), 1.0f - (x / ((float) getWidth())));
            invalidate = true;
        }
    ...
    private boolean stopGlowAnimations(MotionEvent e) {
        ...
        if (!(edgeEffect == null || EdgeEffectCompat.getDistance(edgeEffect) == 0.0f || canScrollHorizontally(-1))) {
            EdgeEffectCompat.onPullDistance(this.mLeftGlow, 0.0f, 1.0f - (e.getY() / ((float) getHeight())));
            stopped = true;
        }
        EdgeEffect edgeEffect2 = this.mRightGlow;
        if (!(edgeEffect2 == null || EdgeEffectCompat.getDistance(edgeEffect2) == 0.0f || canScrollHorizontally(1))) {
            EdgeEffectCompat.onPullDistance(this.mRightGlow, 0.0f, e.getY() / ((float) getHeight()));
            stopped = true;
        }
        EdgeEffect edgeEffect3 = this.mTopGlow;
        if (!(edgeEffect3 == null || EdgeEffectCompat.getDistance(edgeEffect3) == 0.0f || canScrollVertically(-1))) {
            EdgeEffectCompat.onPullDistance(this.mTopGlow, 0.0f, e.getX() / ((float) getWidth()));
            stopped = true;
        }
        EdgeEffect edgeEffect4 = this.mBottomGlow;
        ...
        EdgeEffectCompat.onPullDistance(this.mBottomGlow, 0.0f, 1.0f - (e.getX() / ((float) getWidth())));
        return true;
    }
            ...
            case 2:
                ...
                if (index >= 0) {
                    ...
                    if (this.mScrollState != 1) {
                        ...
                        if (canScrollVertically) {
                            if (dy2 > 0) {
                                dy2 = Math.max(0, dy2 - this.mTouchSlop);
                            } else {
                                dy2 = Math.min(0, this.mTouchSlop + dy2);
                            }
                            if (dy2 != 0) {
                                startScroll = true;
                            }
                    ...
                    if (this.mScrollState != i) {
                        ...
                        if (scrollByInternal(canScrollHorizontally ? dx2 : 0, canScrollVertically ? dy : 0, motionEvent, 0)) {
                            getParent().requestDisallowInterceptTouchEvent(true);
                        }
                        GapWorker gapWorker = this.mGapWorker;
                        if (!(gapWorker == null || (dx2 == 0 && dy == 0))) {
                            gapWorker.postFromTraversal(this, dx2, dy);
                            break;
                        }
                    }
                } else {
    ...
    public void removeDetachedView(View child, boolean animate) {
        ...
        if (vh != null) {
            ...
            throw new IllegalArgumentException("No ViewHolder found for child: " + child + exceptionLabel());
        }
        child.clearAnimation();
    ...
    public void offsetPositionRecordsForMove(int from, int to) {
        ...
        for (int i2 = 0; i2 < childCount; i2++) {
            ...
            if (holder != null && (i = holder.mPosition) >= start && i <= end) {
                if (sVerboseLoggingEnabled) {
                    Log.d(TAG, "offsetPositionRecordsForMove attached child " + i2 + " holder " + holder);
                }
    ...
    private float getSplineFlingDistance(int velocity) {
        double l = Math.log((double) ((((float) Math.abs(velocity)) * INFLEXION) / (this.mPhysicalCoef * SCROLL_FRICTION)));
        float f = DECELERATION_RATE;
        return (float) (((double) (this.mPhysicalCoef * SCROLL_FRICTION)) * Math.exp((((double) f) / (((double) f) - 1.0d)) * l));
    }
    ...
    class ViewFlinger implements Runnable {
        ...
        public void run() {
            ...
            if (scroller.computeScrollOffset()) {
                ...
                this.mLastFlingX = x;
                this.mLastFlingY = y;
                int unconsumedX = RecyclerView.this.consumeFlingInHorizontalStretch(x - this.mLastFlingX);
                int unconsumedY = RecyclerView.this.consumeFlingInVerticalStretch(y - this.mLastFlingY);
                int consumedX = 0;
                int consumedY = 0;
                RecyclerView recyclerView2 = RecyclerView.this;
    ...
    public static class RecycledViewPool {
        ...
        public void clear() {
            for (int i = 0; i < this.mScrap.size(); i++) {
                ...
                Iterator<ViewHolder> it = data.mScrapHeap.iterator();
                while (it.hasNext()) {
        ...
        public void putRecycledView(ViewHolder scrap) {
            ...
            } else if (!RecyclerView.sDebugAssertionsEnabled || !scrapHeap.contains(scrap)) {
                scrap.resetInternal();
                scrapHeap.add(scrap);
            } else {
        ...
        /* access modifiers changed from: package-private */
        public void detachForPoolingContainer(@NonNull Adapter<?> adapter, boolean isBeingReplaced) {
            ...
            if (this.mAttachedAdaptersForPoolingContainer.size() == 0 && !isBeingReplaced) {
                for (int keyIndex = 0; keyIndex < this.mScrap.size(); keyIndex++) {
                    ...
                    ArrayList<ViewHolder> scrapHeap = sparseArray.get(sparseArray.keyAt(keyIndex)).mScrapHeap;
    ...
    public final class Recycler {
        ...
        public ViewHolder tryGetViewHolderForPositionByDeadline(int position, boolean dryRun, long deadlineNs) {
            ...
            if (RecyclerView.this.mState.isPreLayout() && holder.isBound()) {
                ...
                if (!RecyclerView.sDebugAssertionsEnabled || !holder.isRemoved()) {
                    bound = tryBindViewHolderByDeadline(holder, RecyclerView.this.mAdapterHelper.findPositionOffset(i), position, deadlineNs);
                } else {
                    throw new IllegalStateException("Removed holder should be bound and it should come here only in pre-layout. Holder: " + holder + RecyclerView.this.exceptionLabel());
                }
            }
            ViewGroup.LayoutParams lp = holder.itemView.getLayoutParams();
            if (lp == null) {
        ...
        public void recycleCachedViewAt(int cachedViewIndex) {
            if (RecyclerView.sVerboseLoggingEnabled) {
                Log.d(RecyclerView.TAG, "Recycling cached view at index " + cachedViewIndex);
            }
        ...
        public void recycleViewHolderInternal(ViewHolder holder) {
            ...
            if (holder.isScrap() || holder.itemView.getParent() != null) {
                ...
                boolean forceRecycle = adapter != null && transientStatePreventsRecycling && adapter.onFailedToRecycleView(holder);
                boolean cached = false;
                boolean recycled = false;
                if (!RecyclerView.sDebugAssertionsEnabled || !this.mCachedViews.contains(holder)) {
                ...
                throw new IllegalArgumentException("cached view received recycle internal? " + holder + RecyclerView.this.exceptionLabel());
            } else {
            ...
            if (dryRun || (view = RecyclerView.this.mChildHelper.findHiddenNonRemovedView(position)) == null) {
                ...
                while (i2 < cacheSize) {
                    ...
                    if (holder2.isInvalid() || holder2.getLayoutPosition() != position || holder2.isAttachedToTransitionOverlay()) {
                        ...
                        if (RecyclerView.sVerboseLoggingEnabled) {
                            Log.d(RecyclerView.TAG, "getScrapOrHiddenOrCachedHolderForPosition(" + position + ") found match in cache: " + holder2);
                        }
                        return holder2;
                    }
                }
        ...
        public void dispatchViewRecycled(@NonNull ViewHolder holder) {
            ...
            if (RecyclerView.sVerboseLoggingEnabled) {
                Log.d(RecyclerView.TAG, "dispatchViewRecycled: " + holder);
            }
        }
        ...
        public void offsetPositionRecordsForMove(int from, int to) {
            ...
            for (int i2 = 0; i2 < cachedCount; i2++) {
                ...
                if (holder != null && (i = holder.mPosition) >= start && i <= end) {
                    ...
                    if (RecyclerView.sVerboseLoggingEnabled) {
                        Log.d(RecyclerView.TAG, "offsetPositionRecordsForMove cached child " + i2 + " holder " + holder);
                    }
                }
            }
        }
        ...
        public void offsetPositionRecordsForInsert(int insertedAt, int count) {
            ...
            for (int i = 0; i < cachedCount; i++) {
                ...
                if (holder != null && holder.mPosition >= insertedAt) {
                    if (RecyclerView.sVerboseLoggingEnabled) {
                        Log.d(RecyclerView.TAG, "offsetPositionRecordsForInsert cached " + i + " holder " + holder + " now at position " + (holder.mPosition + count));
                    }
        ...
        public void offsetPositionRecordsForRemove(int removedFrom, int count, boolean applyToPreLayout) {
            ...
            for (int i = this.mCachedViews.size() - 1; i >= 0; i--) {
                ...
                if (holder != null) {
                    ...
                    if (i2 >= removedEnd) {
                        if (RecyclerView.sVerboseLoggingEnabled) {
                            Log.d(RecyclerView.TAG, "offsetPositionRecordsForRemove cached " + i + " holder " + holder + " now at position " + (holder.mPosition - count));
                        }
        ...
        private void poolingContainerDetach(Adapter<?> adapter, boolean isBeingReplaced) {
            RecycledViewPool recycledViewPool = this.mRecyclerPool;
            if (recycledViewPool != null) {
                recycledViewPool.detachForPoolingContainer(adapter, isBeingReplaced);
            }
        }
    ...
    public static abstract class Adapter<VH extends ViewHolder> {
        ...
        public final void bindViewHolder(@NonNull VH holder, int position) {
            ...
            if (RecyclerView.sDebugAssertionsEnabled) {
                if (holder.itemView.getParent() == null && ViewCompat.isAttachedToWindow(holder.itemView) != holder.isTmpDetached()) {
                    throw new IllegalStateException("Temp-detached state out of sync with reality. holder.isTmpDetached(): " + holder.isTmpDetached() + ", attached to window: " + ViewCompat.isAttachedToWindow(holder.itemView) + ", holder: " + holder);
                } else if (holder.itemView.getParent() == null && ViewCompat.isAttachedToWindow(holder.itemView)) {
                    throw new IllegalStateException("Attempting to bind attached holder with no parent (AKA temp detached): " + holder);
                }
            }
    ...
    public static abstract class LayoutManager {
        ...
        private void addViewInt(View child, int index, boolean disappearing) {
            ...
            if (lp.mPendingInvalidate) {
                if (RecyclerView.sVerboseLoggingEnabled) {
                    Log.d(RecyclerView.TAG, "consuming pending invalidate on child " + lp.mViewHolder);
                }
        ...
        private void scrapOrRecycleView(Recycler recycler, int index, View view) {
            ...
            if (viewHolder.shouldIgnore()) {
                if (RecyclerView.sVerboseLoggingEnabled) {
                    Log.d(RecyclerView.TAG, "ignoring view " + viewHolder);
                }
            } else if (!viewHolder.isInvalid() || viewHolder.isRemoved() || this.mRecyclerView.mAdapter.hasStableIds()) {
                detachViewAt(index);
    ...
    public static abstract class ViewHolder {
        ...
        public final void setIsRecyclable(boolean recyclable) {
            ...
            if (RecyclerView.sVerboseLoggingEnabled) {
                Log.d(RecyclerView.TAG, "setIsRecyclable val:" + recyclable + ":" + this);
            }
        }
